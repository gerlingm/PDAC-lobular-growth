---
title: "Quantification of tumor-tissue interactions, validation of machine-learning based annotations and lobular NGFR intensity"
...
# Tissue interactions with tumor cell detections
...
setwd("/Users/sara.soderqvist/Library/CloudStorage/OneDrive-KarolinskaInstitutet/Writing_/Adaptive PDAC paper/Code/source data files to read in/ML tumor interactions") # adjust the working directory

# distance frame with invasion front
dist.frame.original <- read.table('./tumor_tissue_interactions_ngfr_intensity_tumor.tsv', sep = '\t', header = T)
...
dist_frame <- dist.frame.original %>%
  select(Image, Class, Object.ID, ends_with('AP..Mean'), starts_with('Distance'))%>%
  data.frame()

# Simplify column and variable names
names <- colnames(dist_frame[,5:ncol(dist_frame)])
names <- gsub("Distance.to.annotation.with.", "", names) 
names <- gsub(".µm", "", names)
...
dist_frame <- dist_frame %>%
  select(PKR, ROI, Object.ID, AP_mean, Langerhans, acinar, nerve, vessel, ADM, ductal, immune, tumor, fibroblasts, ends_with('Front')) %>%
  data.frame()
...
## Subset the data
### Direct interactions according to distance to invasion front within lobular invasion ROIs
...
direct_interactions <- dist_frameL.direct%>%
  select(Object.ID, PKR_ROI, to_class)
...
### Visualise interactions according to distance to invasion front
...
InvFront.plot <-  invfront.data%>%
  select(PKR_ROI,Interaction, Distance_rounded)%>%
...
colors <- c('fibroblasts' = "#38f0f0",'ADM'="#007cff", 'acinar'="#c714f4",'Langerhans'="#1bf200", 'vessel'="#0000ff", 'ductal'="#f78120", 'immune'="#9a6319", 'nerve'="#008100")
...
### Direct interactions within lobular and stromal invasion ROIs
...
#### Comparision of AI based interactions to manual pins
...
# import the data
pins <- read.table("./manual_pins.tsv", header = TRUE, sep = "\t", dec = ".")
...
pkr51.pins <- pins%>%
  ...
  select(Parent, Class, Num.points)%>%
...
pin.pies # Supplementary Fig 11b

```

#### Visualistion of AI based interactions in stroma and lobules
...
ggplot(data = Sankey.data,
  ...
  guides(fill = guide_legend(title = "Cell class")) # Fig 3c
```

#### Visualistion of AI based interactions in stroma and lobules - no fibrosis
...
# NGFR intensity of expanded area from acinar, acinar-to-ductal-metaplasia (ADM) and tumor cell detections in lobuli

## Import the acinar and ADM based intensity measurements

```{r import-ngfr-data, results='hold'}
setwd("/Users/sara.soderqvist/Library/CloudStorage/OneDrive-KarolinskaInstitutet/Writing_/Adaptive PDAC paper/Code/source data files to read in/NGFR IHC") # adjust the working directory

# acinar cell detection based intensity measurements based on the surrounding tissue
acinar.original <- read.table('./ngfr_intensity_acinar.tsv', sep = '\t', header = T)
...
ADM.original <- read.table('./ngfr_intensity_ADM.tsv', sep = '\t', header = T)
...
ngfr.data <- ngfr.data %>%
  select(Image, Class, cell_type, ends_with(c('AP..Mean', 'fibroblasts.µm')))%>%
  data.frame()
...
## Visualization of NGFR intensity of the surrounding tissue of tumor cells in lobular invasion areas
...
tumor_intensities <- invfront.data%>%
  ...
  select(Object.ID, PKR, PKR_ROI, distance_to_invf, AP_mean)
...
lines <- ngfr.data%>%
  ...
  select(cell_type, AP_norm) %>% #compute mean
  group_by(cell_type)%>%
